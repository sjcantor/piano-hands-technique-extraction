...
class lstm_encoder(nn.Module):
    ...
    def forward(self, x, x_lengths, edge_list=None):
        # pdb.set_trace()
        x_packed = packer(x.float(), x_lengths.cpu().numpy(), batch_first=True, enforce_sorted=False)
        # pdb.set_trace()
    ...
    def forward(self, x, x_lengths, edge_list, fingers=None, ratio_teaching_forcing=0.7):
        ...
        if teaching_forcing:
            ...
            prev_gt = torch.cat(
                ...
                dim=1)
            prev_gt_packed = packer(prev_gt, x_lengths.cpu().numpy(), batch_first=True, enforce_sorted=False)
            ...
            concated_data_packed = packer(concated_data.float(), x_lengths.cpu().numpy(), batch_first=True, enforce_sorted=False)
